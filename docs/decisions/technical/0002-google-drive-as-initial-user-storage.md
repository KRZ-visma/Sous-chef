...
The project currently runs as a static Vite + React site and avoids backend infrastructure. Upcoming data needs include user-managed week plans and potentially recipe pictures. We need a first storage approach that can be adopted quickly without introducing a backend service or committing to a larger data platform before real usage constraints are known.
...
3. Keep this as a pragmatic first step focused on low setup overhead and minimal tooling decisions.
4. Re-evaluate and move to **Firebase** if Drive-based storage causes product or technical friction.
